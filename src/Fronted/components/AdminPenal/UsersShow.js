...
const deleteHender=(item)=>{
  ...
  return (
    <div className="boxDivCard">
      ...
      {item?.image_src ? 
        <img src={`http://localhost:4000/${item?.image_src}`} alt="Avatar" style={{ width: "100px" }} />
        :
      ...
      <div className="nameDivCard">
       ...
       <h4>Reporting to</h4>
        ...
        <div className="textDiv">
          <button   onClick={()=> uditUser(item)} className="textDivBntt" disabled={hedalState.show}>
            <img src={edit} alt="Avatar"  style={{ width: "25px" , margin:"20px 0 0 0" }} />
          </button>
          <button className="textDivBntt">
          <img src={delete_img} alt="Avatar" onClick={()=> deleteHender(item)} style={{ width: "40px" , margin:"20px 0 0 0" }} />
          </button>
          </div>
        </div>          
      </div>
